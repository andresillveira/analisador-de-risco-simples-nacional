...
| **CFOPs de Transporte (Saídas)** (ex: `5.351`, `6.356`) | Transporte | **Saída:** Classificado como `"servicos"` (Serviços Prestados). Soma ao Faturamento total. |
...
| **CFOPs de Uso e Consumo** (ex: `1.556`, `2.556`) | Uso ou Consumo | Computado como despesas operacionais no **Inciso IX** (120%). |
...
*   **Correção na Regra de Negócio de Serviços de Transporte (Nova Regra de Negócio):** Refatoramos a função `classify_cfop_row` no backend modular (`risk_service.py`) e monolítico (`app.py`) para aplicar uma heurística dinâmica baseada no primeiro dígito do CFOP quando a categoria for `"Transporte"`. Assim, os CFOPs iniciados em `5`, `6` ou `7` (Saídas) passam a ser classificados como `"servicos"` (faturamento), enquanto os iniciados em `1`, `2` ou `3` (Entradas) permanecem como `"outras"` (despesas operacionais). Isso resolve a distorção para empresas transportadoras, cujas saídas de frete estavam sendo tratadas incorretamente como despesas.
